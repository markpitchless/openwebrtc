/*
 * The client example with stdin and out signalling.
 */
...
static void send_offer()
{
    ...
    for (item = media_sessions; item; item = item->next) {
        ...
        json_builder_add_string_value(builder, fingerprint);
        json_builder_set_member_name(builder, "setup");
        json_builder_add_string_value(builder, "actpass");
        json_builder_end_object(builder); /* dtls */
...
{
    g_print("candidate_from_positioned_json_reader:\n");
...
{
    g_print("handle_remote_candidate:\n");
    JsonParser *parser;
    JsonReader *reader;
    gint index;
    GList *media_sessions;
